...
const OLD_BEHAVIOUR: bool = false;
...
impl CHIP8 {
    ...
    pub fn load_program(&mut self, program: &[u8]) {
        (&mut self.ram[0x200..(program.len() + 0x200)]).copy_from_slice(program);
    }

    pub fn update(&mut self, input: CHIP8Input, display: &mut Display) -> CHIP8Output {
        let instruction = u16::from_be_bytes((&self.ram[self.pc..=self.pc + 1]).try_into().unwrap());
        self.pc += 2;
        ...
        match first_nibble {
            ...
            0x5 => {
                match fourth_nibble {
                    0x0 => if self.vx_reg[second_nibble] == self.vx_reg[third_nibble] { self.pc+=2; }
                    _ => panic!("unknown opcode")

                }
            }
            0x6 => {
                self.vx_reg[second_nibble as usize] = nn;
            }
            0x7 => {
                self.vx_reg[second_nibble as usize] = self.vx_reg[second_nibble as usize].wrapping_add(nn);
            }
            0x8 => match fourth_nibble {
                ...
                0x4 => {
                    let overflowed;
                    (self.vx_reg[second_nibble], overflowed) = self.vx_reg[second_nibble].overflowing_add(self.vx_reg[third_nibble]);
                    self.vx_reg[0xf] = if overflowed {
                        1
                    } else { 0 };
                }
                0x5 => {
            ...
            0x9 => 
                match fourth_nibble {
                    0x0 => if self.vx_reg[second_nibble as usize] != self.vx_reg[third_nibble as usize] { self.pc+=2; }
                    _ => panic!("unknown opcode")
            ...
            0xD => {
                ...
                let (mut x, mut y) = (
                    self.vx_reg[second_nibble as usize] % 64,
                    self.vx_reg[third_nibble as usize] % 32,
                );
                self.vx_reg[15] = 0;

                for i in 0..fourth_nibble as usize {
                    x = self.vx_reg[second_nibble as usize] % 64; 
            ...
            0xF => match nn {
                0x1E => {
                    ...
                    let overflowing;
                    (self.i_reg, overflowing) = self.i_reg.overflowing_add(self.vx_reg[second_nibble] as u16);
                    if !OLD_BEHAVIOUR {
                    self.vx_reg[0xF] = if overflowing {
                        ;1
                    } else { 0}
                    }
                },
                ...
                0x55 => {
                    for idx in 0..=second_nibble {
                        self.ram[self.i_reg as usize + if !OLD_BEHAVIOUR { idx } else { 0 }] = self.vx_reg[idx];
                        if OLD_BEHAVIOUR {
                            self.i_reg += 1;
                        }
                    }
                ...
                0x65 => {
                    for idx in 0..=second_nibble {
                        self.vx_reg[idx] = self.ram[self.i_reg as usize + if !OLD_BEHAVIOUR { idx } else { 0 }];
                        if OLD_BEHAVIOUR {
                            self.i_reg += 1;
                        }
                    }
